...
namespace BoxLifting
{
    public partial class MultiPlayerGame : Form
    {
        ...
        private int PortNumber = 1014;
        public static string ip;
        public static string playerName;
        public static bool serverSide;
        public int[] karegar = new int[2];
        public int[] stopzone = new int[2];
        public int[] karegarMulti = new int[2];
        public int[] stopzoneMulti = new int[2];
        bool chkpress = true;
        int level = 1;
        bool escape = false;
        string playerNameMulti;
        readonly int _ServerPort = 1014;
        string _ServerIPAddress = "127.0.0.1";
        bool serverGet;
        public MultiPlayerGame()
        {
            InitializeComponent();
            playerNameMulti = playerName;
            _ServerIPAddress = ip;
            serverGet = serverSide;
        ...
        public void timer()
        {
            ...
            timer1.Interval = 10; // 10 mili second
            timer1.Start();
        }
        ...
        private void MultiPlayerGame_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                chkpress = false;
                if (((Keys)e.KeyValue == Keys.Left) && (escape == false))//left
                {
                    moving(1, "L");
                    ...
                    for (int i = 0; i <= 5; i++)
                    {
                        // row.colomn_row2.colomn2_...
                        sendtext = sendtext + dt.Rows[i].ToString() + "." + dt.Columns[i].ToString() + "_";
                    }
                    byte[] buffer = Encoding.ASCII.GetBytes(sendtext);
                    MySocket.Send(buffer);
                }
                if (((Keys)e.KeyValue == Keys.Up) && (escape == false))//up
                {
                    moving(1, "U");
                ...
                if (((Keys)e.KeyValue == Keys.Right) && (escape == false))//right
                {
                    moving(1, "R");
                ...
                if (((Keys)e.KeyValue == Keys.Down) && (escape == false))//down
                {
                    moving(1, "D");
                ...
                if (((Keys)e.KeyValue == Keys.Escape) && (escape == false))//Esc
                {
                    escape = true;
        ...
        public void Server()
        {
            try
            {
                ...
                IPEndPoint MyEndPoint = new IPEndPoint(IPAddress.Any, PortNumber);
                MySocket.Bind(MyEndPoint);
                MySocket.Listen(1);
                MySocket.BeginAccept(new AsyncCallback(Server_Gate), null); // Method e Asankron
                // Tabe Marbut Be Server
                ShowString("l" + "Creteded Host");
            }
            catch
            {
                ShowString("l" + "Problem in Created");
            }
        }

        private void Server_Gate(IAsyncResult asyn)
        {
            MySocket = MySocket.EndAccept(asyn);
            ShowString("l" + "Connect Client");
            Wait4Data();

        } // In Method Vaghti Ke Client Be Server Connect Mishe Tavasote Methode Asankron Seda Zade Mishe

        private void Wait4Data() // Tabe ii Ke Dakhele Un Methode Asankron Baraye Daryafte Ettelaat Seda Zade Mishe
        {
            //try
            //{
            if (MyCallBack == null) MyCallBack = new AsyncCallback(ReciveData);
            dataBuffer = new byte[10526];
            MySocket.BeginReceive(dataBuffer, 0, dataBuffer.Length, SocketFlags.None, MyCallBack, MySocket);
        ...
        private void ReciveData(IAsyncResult asyn)// In Method Vaghti Ettelaati Daryaft Mishe Tavasote Methode Asankron Seda Zade Mishe
        {
            try
            {
                ...
                string file = new string(chars);// ettelate Daryafi Tu string e file zakhire mishe
                ShowString("r" + file);
                Wait4Data();
            }
            catch { ShowString("l" + "Client Left !"); }
        }
        delegate void ShowStringCallback(string text);
        private void ShowString(string myText)
        {
